...
- **Popular domains are never blocked.** Everything is checked against the
  ...
  feed claims about it. Subdomains are covered too, so a payload hosted on a
  major CDN or code host won't get the whole platform blocked.
- **Shared hosting is handled properly.** Free dynamic-DNS and static-hosting
  providers (`duckdns.org`, `github.io`, `workers.dev`) are popular platforms
  whose subdomains belong to unrelated people. The platform stays unblocked; an
  individual malicious subdomain does not.
- **Critical infrastructure is permanently protected** — including the URLs
  these lists are served from, so subscribing to a list can never break its own
  updates.
